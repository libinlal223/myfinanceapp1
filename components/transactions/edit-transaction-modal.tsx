...
import { Transaction, TransactionType, TYPE_CONFIG, Category } from "@/lib/types";
...
export default function EditTransactionModal({ transaction: tx, onClose }: Props) {
  ...
  const [activeTab, setActiveTab] = useState<TransactionType>(tx.type);
    ...
    <div className="fixed inset-0 z-50 flex items-end md:items-center justify-center animate-fade-in">
      ...
      <div className="relative w-full md:max-w-md bg-card border border-border rounded-t-2xl md:rounded-2xl shadow-2xl animate-slide-up max-h-[90dvh] overflow-y-auto">
        <div className="flex items-center justify-between p-4 border-b border-border">
          <h2 className="font-semibold text-base">Edit Transaction</h2>
          <button onClick={onClose} className="w-8 h-8 rounded-lg flex items-center justify-center text-muted-foreground hover:text-foreground hover:bg-accent transition-all" aria-label="Close">
            <X size={16} />
          </button>
        </div>
          ...
          {TABS.map((tab) => {
            ...
            return (
              <button key={tab} onClick={() => setActiveTab(tab)}
                className={cn("flex-1 py-3 text-xs font-semibold uppercase tracking-wide transition-all border-b-2",
                  activeTab === tab ? `${cfg.color} border-current` : "text-muted-foreground border-transparent hover:text-foreground"
                )}>
                {cfg.label}
              </button>
            );
        ...
        <form onSubmit={handleSubmit} className="p-4 space-y-4">
          <div>
            ...
            <div className="relative">
              ...
              <input type="number" required min="0.01" step="0.01" value={amount} onChange={(e) => setAmount(e.target.value)}
                className={cn("w-full h-12 pl-8 pr-3 rounded-lg bg-input border border-border text-lg font-mono-num font-semibold focus:outline-none focus:ring-2 focus:ring-ring transition-colors", config.color)} />
            </div>
          </div>

          {activeTab === "expense" && categories.length > 0 && (
            <div>
              <label className="block text-xs font-medium text-muted-foreground mb-1.5 uppercase tracking-wide">Category</label>
              <div className="grid grid-cols-4 gap-1.5">
                {categories.map((cat) => (
                  <button key={cat.id} type="button" onClick={() => setCategoryId(cat.id === categoryId ? "" : cat.id)}
                    className={cn("flex flex-col items-center gap-1 p-2 rounded-lg text-xs transition-all border",
                      categoryId === cat.id ? "bg-primary/15 border-primary text-primary" : "border-border hover:border-muted-foreground text-muted-foreground hover:text-foreground"
                    )}>
          ...
          <button type="submit" disabled={isPending || !amount}
            className="w-full h-11 rounded-lg bg-primary text-primary-foreground font-semibold text-sm hover:bg-primary/90 disabled:opacity-50 disabled:cursor-not-allowed transition-all flex items-center justify-center gap-2">
